...
let zeroes=[];
console.log('Sorting...');
for (let count=0; count<supplyChanges.length; count++) {
  if (supplyChanges[count]>=1) {
    positives.push(supplyChanges[count]);
  }
  else if (supplyChanges[count]<=-1) {
    negatives.push(supplyChanges[count]);
  }
  else{
    zeroes.push(supplyChanges[count]);
  }
